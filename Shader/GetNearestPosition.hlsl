...
{
    float4 sampled = positionTexture.Sample(my_point_clamp_sampler,float2(.5 / interactors, 0));
    float3 closest = sampled;
    float closestSqrDst = SqrDst(closest, position);    
    for(int i = 1; i < interactors; i++)
    {
        float4 sampled = positionTexture.Sample(my_point_clamp_sampler,float2((i+.5) / interactors, 0));
        float3 cur = sampled;
